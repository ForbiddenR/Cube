...
import (
	...
	"github.com/ForbiddenR/cube/scheduler"
...
func (m *Manager) SelectWorker(t task.Task) (*node.Node, error) {
	candidates := m.Scheduler.SelectCandidateNodes(t, m.WorkerNdoes)
	if candidates == nil {
	...
	if m.Pending.Len() > 0 {
		...
		if ok {
			...
			log.Printf("invalid request: existing task %s is in state %v and cannot transition to the completed state\n",
				persistedTask.ID.String(), persistedTask)
			return
		}
...
func (m *Manager) restartTask(t *task.Task) {
	...
	if resp.StatusCode != http.StatusCreated {
		...
		log.Printf("Response err (%d): %s", e.HttpStatusCode, e.Message)
		return
	}
